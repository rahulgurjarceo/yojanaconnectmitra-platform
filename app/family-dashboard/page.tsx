...
export default function FamilyDashboardPage() {
  ...
  const modules = state.data.family.modules as string[];
  return <main className="min-h-screen bg-slate-950 px-5 py-10 text-white"><div className="mx-auto max-w-5xl"><p className="text-xs font-black uppercase tracking-[0.25em] text-cyan-300">YCM ONE • Family 360</p><div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-4xl font-black">Family Dashboard</h1><p className="mt-2 text-slate-400">Private family scope: {state.data.family.familyId}</p></div><span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs font-bold text-emerald-300">Verified session</span></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{modules.map(module => <section key={module} className="rounded-3xl border border-white/10 bg-white/5 p-6"><h2 className="font-bold capitalize">{module.replaceAll('-', ' ')}</h2><p className="mt-2 text-sm text-slate-400">Available only within this authenticated family scope.</p></section>)}</div></div></main>;
}

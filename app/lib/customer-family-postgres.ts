...
function mapRow(row: Record<string, unknown>): FamilyRecord {
  return {
    familyId: String(row.family_id), status: row.status as FamilyRecord['status'], fullName: String(row.full_name),
    mobile: String(row.mobile), country: String(row.country), createdAt: new Date(String(row.created_at)).toISOString(),
...
export class PostgresCustomerFamilyRepository implements CustomerFamilyRepository {
  ...
  async create(input: Omit<FamilyRecord, 'createdAt' | 'updatedAt'>): Promise<FamilyRecord> {
    ...
    const rows = await this.sql`
      INSERT INTO ycm_families (family_id, plan_name, amount_paise, currency, validity_years, status, full_name, mobile, country)
      VALUES (${input.familyId}, 'Family Registration', 9900, 'INR', 2, ${input.status}, ${input.fullName}, ${input.mobile}, ${input.country})
